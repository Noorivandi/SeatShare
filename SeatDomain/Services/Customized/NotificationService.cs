...
namespace SeatDomain.Services
{
    ...
    public partial class NotificationService : ReadOnlySupportAuditerService<Notification, IRepository<Notification>, IReadOnlyRepository<Notification>, INotificationService>, INotificationService
    {
        ...
        public override Notification GetEntity(object key, Timestamp version)
        {
            var userSrv = ServiceFactory.Create<IAspNetUserService>();

            var notification = base.GetEntity(key, version);

            var user = userSrv.FindByName(notification.Owner);
            if (user.EmailConfirmed)
                notification.Email = user.Email;
            if (user.PhoneNumberConfirmed)
                notification.PhoneNo = user.PhoneNumber;
            notification.PhoneNumberConfirmed = user.PhoneNumberConfirmed;
            notification.EmailConfirmed = user.EmailConfirmed;
            return notification;
        }
