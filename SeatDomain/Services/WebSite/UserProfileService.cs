namespace SeatDomain.Services
{
    ...
    public partial class UserProfileService : ReadOnlySupportBaseOfService<UserProfile, IUserProfileService>, IUserProfileService
    {
        ...
        public override object Save(UserProfile entity)
        {
            ...
            return new SwitchWindowResponse()
            {
                Message = "اطلاعات پروفایل به روزرسانی شد",
                Success = true,
                CurrentWindowName = nameof(UserProfile),
                ReplaceCurrentWindow = true,
                NewWindowName = nameof(UserProfile),
                MessageType = ActionResponse.MessageTypes.Info,
                WindowParameters = new NameValueCollection { { "id", entity.ProfilePK.ToString() }, { "sv", "1" }, { "mode", "DataEntry" } }
            };
        }
