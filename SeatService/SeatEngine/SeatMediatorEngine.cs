...
namespace SeatService.SeatServiceEngine
{
    public class SeatServiceMediatorEngine : IDisposable
    {
        ...
        private async Task addClientTypesAsync()
        {
            var changed_instances = Online.Data.Values.Where(x => x.IsValid() && x.IsTypeClientChanges());

            var clientTypes = changed_instances.Select(x => new ClientType(x)).ToList();

            if (clientTypes.Count == 0) return;

            var ctSrv = StaticServiceFactory.Create<IClientTypeService>();
            var bdSrv = StaticServiceFactory.Create<IBigDealService>();

            await ctSrv.BulkInsertAsync(clientTypes);

            await bdSrv.ExtractBigDealsAsync(clientTypes);
